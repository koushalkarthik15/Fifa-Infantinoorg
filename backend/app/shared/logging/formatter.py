...
from app.shared.logging.request_context import get_request_id, get_request_method, get_request_path

SENSITIVE_FIELDS = {
    "password", "token", "authorization", "secret", "api_key", "cookie", "access_token", "refresh_token"
}

def sanitize_dict(data: Dict[str, Any]) -> Dict[str, Any]:
    sanitized = {}
    for k, v in data.items():
        if isinstance(k, str) and any(sensitive in k.lower() for sensitive in SENSITIVE_FIELDS):
