import json
import logging
from app.features.navigation.repositories.navigation_repository import NavigationRepository
from app.features.navigation.schemas.navigation import (
    VenueNavigationRequest, VenueNavigationResponse, RouteSegmentDTO,
    TransportationRequest, TransportationResponse
)
from app.features.navigation.prompts.navigation import TRANSPORTATION_RECOMMENDATION_PROMPT_TEMPLATE
...
class NavigationService:
    ...
    async def get_venue_route(self, request: VenueNavigationRequest) -> VenueNavigationResponse:
        ...
        if request.destination_category and not destination_name:
            locations = self.repository.get_locations_by_category(request.destination_category)
            if locations:
        ...
        try:
            maps_result = self.maps_provider.get_directions(request.origin, destination_name)
            ...
            segments = [{"instruction": f"Head towards {destination_name}", "distance_meters": 0, "duration_seconds": 0}]
            total_duration = 0
    ...
    async def get_transportation_recommendation(self, request: TransportationRequest) -> TransportationResponse:
        try:
            # Let's get walking directions distance/time as a baseline
            maps_result = self.maps_provider.get_directions(request.origin, request.destination)
