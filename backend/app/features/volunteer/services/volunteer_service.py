...
from app.features.volunteer.prompts.volunteer import INCIDENT_CLASSIFICATION_PROMPT_TEMPLATE
...
class VolunteerService:
    ...
    async def report_incident(self, request: IncidentRequest) -> IncidentResponse:
        ...
        try:
            ...
            priority = "High"
            suggested_actions = ["Dispatch nearest available staff immediately to assess the situation."]

        assigned_volunteer = get_volunteer_for_category(category)

        # Log metadata only, never verbatim description
        logger.info(f"Incident processed: Category={category}, Priority={priority}, Assigned={assigned_volunteer}")
