...
from app.features.accessibility.schemas.accessibility import (
    ...
    AccessibleRouteRequest, AccessibleRouteResponse, RouteSegmentDTO,
...
class AccessibilityService:
    ...
    async def get_assistant_response(self, request: AssistantRequest) -> AssistantResponse:
    ...
    async def get_accessible_route(self, request: AccessibleRouteRequest) -> AccessibleRouteResponse:
        try:
            ...
            segments = [{"instruction": f"Head directly to {request.destination}", "distance_meters": 0, "duration_seconds": 0}]
            total_duration = 0
        ...
        if request.needs_wheelchair:
            ...
            try:
                ai_response = self.ai_provider.generate_content(prompt)
                result = json.loads(clean_json(ai_response))
                accessibility_context = result.get("accessibility_context", "Proceed with standard accessible routing.")
            except Exception as e:
    ...
    async def analyze_vision(self, image_bytes: bytes, mime_type: str) -> VisionResponse:
        prompt = VISION_ANALYSIS_PROMPT_TEMPLATE
        try:
            ai_response = self.ai_provider.generate_content(prompt, image_bytes=image_bytes, mime_type=mime_type)
            result = json.loads(clean_json(ai_response))
