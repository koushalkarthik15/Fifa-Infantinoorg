...
class PredictionRequest(BaseCamelModel):
    ...
    time_offset_minutes: int = Field(
        default=0, description="Minutes from now to predict."
    )


class PredictionResponse(BaseCamelModel):
    prediction_text: str = Field(
        description="Natural language explanation of crowd prediction."
    )
    recommended_action: str = Field(
        description="Suggested action for the user based on crowd prediction."
    )
...
class RouteResponse(BaseCamelModel):
    ...
    crowd_context: str = Field(
        description="Gemini-generated context explaining route adjustments for crowd avoidance."
    )
