...
router = APIRouter(tags=["Crowd AI"])
...
async def get_heatmap(
    ...
    return APIResponse(
        success=True, message="Heatmap retrieved successfully.", data=data
    )
...
async def predict_crowd(
    request: PredictionRequest, service: CrowdService = Depends(get_crowd_service)
) -> APIResponse[PredictionResponse]:
    data = await service.get_prediction(request)
    return APIResponse(
        success=True, message="Prediction retrieved successfully.", data=data
    )
...
async def recommend_route(
    request: RouteRequest, service: CrowdService = Depends(get_crowd_service)
) -> APIResponse[RouteResponse]:
    data = await service.get_route(request)
    return APIResponse(success=True, message="Route augmented successfully.", data=data)
