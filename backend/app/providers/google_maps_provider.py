    ...
    def get_directions(self, origin: str, destination: str, **kwargs) -> dict:
        ...
        while attempt < max_attempts:
            try:
                ...
                attempt += 1
                logger.warning(f"Google Maps API Error, retry {attempt}/{max_attempts}: {str(e)}")
                ...
                raise ExternalServiceError("Failed to fetch directions from Google Maps.")
            except Exception as e:
                attempt += 1
                logger.warning(f"Unexpected Google Maps error, retry {attempt}/{max_attempts}: {str(e)}")
                if attempt < max_attempts:
                    import time
                    time.sleep(delay_seconds)
                    delay_seconds *= 2
                    continue
                logger.error(f"Unexpected error in Google Maps directions after retries: {str(e)}")
                raise ExternalServiceError("Unexpected error communicating with Google Maps.")
