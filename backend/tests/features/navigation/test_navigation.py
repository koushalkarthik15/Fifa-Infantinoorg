...
def setup_navigation_db():
    ...
    with Session(engine) as session:
        ...
        if not existing:
            session.add(
                VenueLocation(
                    name="North Gate", category="Gate", latitude=33.9, longitude=-118.2
                )
            )
            session.add(
                VenueLocation(
                    name="Seat 10", category="Seat", latitude=33.91, longitude=-118.21
                )
            )
            session.commit()
    yield
...
@pytest.mark.asyncio
async def test_venue_navigation(
    async_client: AsyncClient, navigation_setup, monkeypatch
):
    # Mock MapsProvider
    ...
    monkeypatch.setattr(
        "app.features.navigation.services.navigation_service.get_maps_provider",
...
async def test_transportation_recommendation(async_client: AsyncClient, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.features.navigation.services.navigation_service.get_ai_provider",
