...
async def test_green_route(async_client: AsyncClient, monkeypatch):
    class MockMapsProvider:
        def get_directions(self, origin, destination):
            return {
                "segments": [{"instruction": "Walk 500m", "distance_meters": 500, "duration_seconds": 300}],
    ...
    monkeypatch.setattr("app.features.sustainability.services.sustainability_service.get_maps_provider", lambda: MockMapsProvider())
    monkeypatch.setattr("app.features.sustainability.services.sustainability_service.get_ai_provider", lambda: MockAIProvider())
...
async def test_facilities(async_client: AsyncClient):
    response = await async_client.post(
        "/api/v1/sustainability/facilities",
        json={"location": "Section 102"}
    )
    assert response.status_code == 200
    data = response.json()["data"]
