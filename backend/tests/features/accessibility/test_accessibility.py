...
async def test_assistant(async_client: AsyncClient, monkeypatch):
    class MockAIProvider:
        def generate_content(self, prompt, **kwargs):
            return (
                '{"response": "Here to help.", "suggested_actions": ["Call Medical"]}'
            )

    monkeypatch.setattr(
        "app.features.accessibility.services.accessibility_service.get_ai_provider",
        lambda: MockAIProvider(),
    )

    response = await async_client.post(
        "/api/v1/accessibility/assistant", json={"query": "I need help"}
    )
